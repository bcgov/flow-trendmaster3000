...
# Note: the script below comes from the provincial hydrologist Jon Goetz's
#       station filtering script.
...
jon_g_filtering_steps = function(my_path){
  ##### First pass to filter for stations with complete data
  withProgress(message = 'Running one-time station filtering', {
    ...
    stations_filt <- hydat_daily_all %>%
      ...
      left_join(hy_stations(hydat_path = my_path), by = "STATION_NUMBER") %>%
      left_join(hy_stn_regulation(hydat_path = my_path), by = "STATION_NUMBER")
    ...
    ## Maybe a good point to do some automated filtering, e.g. data gaps
    ## cannot be more than X years, trim away records from before any such gaps

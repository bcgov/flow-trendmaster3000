...
server <- function(input, output, session) {
  ...
  stations_sf_to_include = reactive({
  ...
  output$shapes_nrows = renderText({
    req(!is.null(shape_for_map()))
    paste0("Number of shapes: ",nrow(shape_for_map()))
  })

  output$dat = DT::renderDT({
    req(input$user_var_choice)
    stations_sf_with_trend_in_shape() |> head()
    })

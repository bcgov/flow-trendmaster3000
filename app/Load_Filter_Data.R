...
# Load in data ----------------------------------------------------
hydat_downloaded_state = reactive({
  # state is TRUE when hydat database has already been downloaded
  length(list.files(path = hy_dir())) != 0
})

observe({
  if(hydat_downloaded_state() == F){
...
if(!file.exists('filtered_station_list.csv')){
...
flow_dat_daily = tidyhydat::hy_daily_flows(station_list_filtered$STATION_NUMBER) %>%
